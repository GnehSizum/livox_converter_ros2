...
class LivoxConverter {
 public:
  LivoxConverter() {
    ...
    const auto add_field = [this](const std::string &name, const int offset,
    ...
    add_field("y", points_msg->fields.back().offset + sizeof(float),
              PointField::FLOAT32);
    add_field("z", points_msg->fields.back().offset + sizeof(float),
              PointField::FLOAT32);
    add_field("t", points_msg->fields.back().offset + sizeof(float),
              PointField::UINT32);
    add_field("intensity", points_msg->fields.back().offset + sizeof(std::uint32_t),
              PointField::FLOAT32);
    add_field("tag", points_msg->fields.back().offset + sizeof(float),
              PointField::UINT8);
    add_field("line", points_msg->fields.back().offset + sizeof(std::uint8_t),
              PointField::UINT8);
  ...
  PointCloud2ConstPtr convert(const CustomMsg &livox_msg) {
    ...
    for (int i = 0; i < livox_msg.point_num; i++) {
      *reinterpret_cast<float *>(ptr + points_msg->fields[0].offset) =
          livox_msg.points[i].x;
      *reinterpret_cast<float *>(ptr + points_msg->fields[1].offset) =
          livox_msg.points[i].y;
      *reinterpret_cast<float *>(ptr + points_msg->fields[2].offset) =
